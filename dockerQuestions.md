...
> **How to Answer:** "This comes down to who controls Process ID 1 (PID 1) in Linux. PID 1 is the 'boss' process that receives shut-down signals from Docker.
> *   **The Shell Form (String):** e.g., `ENTRYPOINT java -jar app.jar`. This starts a Linux shell (`/bin/sh`) as PID 1, and the Java app becomes its child (PID 2). When Docker sends a `SIGTERM` to shut down, the shell receives it and ignores it. The Java app never gets the message, so Docker eventually `SIGKILL`s the container 10 seconds later, abruptly cutting database connections and risking data corruption.
> *   **The Exec Form (JSON Array):** e.g., `ENTRYPOINT ["java", "-jar", "app.jar"]`. This skips the shell middleman. The Java application itself becomes PID 1. When Docker says to shut down, Spring Boot receives the message directly, safely drains the database connection pool, and gracefully turns off. Always use the Exec form."
